...
app.MapPost("/login", (LoginDTO dto) =>
{
    ...
    bool precisaTrocarSenha =
        !reader.IsDBNull(reader.GetOrdinal("PrecisaTrocarSenha")) &&
        Convert.ToInt32(reader["PrecisaTrocarSenha"]) == 1;
...
app.MapPost("/alterar-senha-obrigatoria", async (HttpRequest request) =>
{
    int? usuarioId = GetUserId(request);
    if (usuarioId == null)
        return Results.Unauthorized();

    var dto = await request.ReadFromJsonAsync<TrocarSenhaDTO>();
    if (dto == null || string.IsNullOrWhiteSpace(dto.NovaSenha))
        return Results.BadRequest("Informe a nova senha.");

    if (dto.NovaSenha.Length < 6)
        return Results.BadRequest("A nova senha deve ter pelo menos 6 caracteres.");

    using var conn = Database.GetConnection();
    conn.Open();

    var verificar = conn.CreateCommand();
    verificar.CommandText = "SELECT PrecisaTrocarSenha FROM Usuarios WHERE Id = @id";
    verificar.Parameters.AddWithValue("@id", usuarioId.Value);

    var precisa = verificar.ExecuteScalar();
    if (precisa == null || Convert.ToInt32(precisa) != 1)
        return Results.BadRequest("A troca obrigatória de senha não está ativa para este usuário.");
    ...
    cmd.CommandText = @"
        ...
        WHERE Id = @id";
    cmd.Parameters.AddWithValue("@senha", HashSenha(dto.NovaSenha));
...
app.MapGet("/me", (HttpRequest request) =>
{
    ...
    return Results.Ok(new
    {
        ...
        precisaTrocarSenha = Convert.ToInt32(reader["PrecisaTrocarSenha"]) == 1
    });
});
...
app.MapPut("/admin/produto/{id}/preco", async (int id, HttpRequest request) =>
{
    ...
    var dados = await request.ReadFromJsonAsync<AlterarPrecoProdutoDTO>();

    if (dados == null || dados.Preco < 0)
    ...
    var alterados = cmd.ExecuteNonQuery();
...
app.MapPost("/comprar", async (HttpRequest request) =>
{
    ...
    foreach (var item in dados)
    {
        ...
        var estoqueCmd = conn.CreateCommand();
        estoqueCmd.CommandText = @"
            INSERT INTO Estoque (ProdutoId, Quantidade)
            VALUES (@produto, -@quantidade)
            ON CONFLICT(ProdutoId) DO UPDATE SET
                Quantidade = Quantidade - @quantidade";
        estoqueCmd.Parameters.AddWithValue("@produto", item.ProdutoId);
        estoqueCmd.Parameters.AddWithValue("@quantidade", item.Quantidade);
...
app.MapGet("/admin/estoque/{produtoId}", (int produtoId, HttpRequest request) =>
{
    if (!IsAdmin(request))
        return Results.Unauthorized();
    ...
    var cmd = conn.CreateCommand();
    cmd.CommandText = @"
        SELECT IFNULL(Quantidade, 0)
        FROM Estoque
        WHERE ProdutoId = @produto";
    cmd.Parameters.AddWithValue("@produto", produtoId);

    var obj = cmd.ExecuteScalar();
    int quantidade = obj == null ? 0 : Convert.ToInt32(obj);

    return Results.Ok(new { quantidade });
});

app.MapPost("/admin/estoque/adicionar", async (HttpRequest request) =>
{
    if (!IsAdmin(request))
        return Results.Unauthorized();

    var dto = await request.ReadFromJsonAsync<EstoqueDTO>();

    if (dto == null || dto.ProdutoId <= 0 || dto.Quantidade <= 0)
        return Results.BadRequest("Produto e quantidade devem ser válidos.");
    ...
    cmd.Parameters.AddWithValue("@produto", dto.ProdutoId);
    cmd.Parameters.AddWithValue("@quantidade", dto.Quantidade);
    cmd.ExecuteNonQuery();

    var saldoCmd = conn.CreateCommand();
    saldoCmd.CommandText = "SELECT Quantidade FROM Estoque WHERE ProdutoId = @produto";
    saldoCmd.Parameters.AddWithValue("@produto", dto.ProdutoId);

    int quantidadeAtual = Convert.ToInt32(saldoCmd.ExecuteScalar());

    return Results.Ok(new { quantidade = quantidadeAtual });
});
...
app.MapPost("/admin/usuario/{id}/resetar-senha", (int id, HttpRequest request) =>
{
    if (!IsAdmin(request))
        return Results.Unauthorized();
    ...
    var existe = conn.CreateCommand();
    existe.CommandText = "SELECT COUNT(*) FROM Usuarios WHERE Id = @id";
    existe.Parameters.AddWithValue("@id", id);

    if (Convert.ToInt32(existe.ExecuteScalar()) == 0)
        return Results.NotFound("Usuário não encontrado.");

    // Gera uma senha temporária de 6 dígitos para o admin repassar ao usuário.
    string senhaTemporaria = RandomNumberGenerator.GetInt32(100000, 1000000).ToString();
    ...
    cmd.CommandText = @"
        ...
        SET SenhaHash = @senha,
            ...
            Token = NULL
        WHERE Id = @id";
    cmd.Parameters.AddWithValue("@senha", HashSenha(senhaTemporaria));
    cmd.Parameters.AddWithValue("@id", id);
    cmd.ExecuteNonQuery();
...
app.MapPost("/admin/adicionar", async (HttpRequest request) =>
{
    ...
    estoqueCmd.CommandText = @"
        ...
        ON CONFLICT(ProdutoId) DO UPDATE SET
            Quantidade = Quantidade - @quantidade";
    estoqueCmd.Parameters.AddWithValue("@produto", dto.ProdutoId);
    estoqueCmd.Parameters.AddWithValue("@quantidade", dto.Quantidade);
    estoqueCmd.ExecuteNonQuery();
